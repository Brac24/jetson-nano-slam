...
# This is a slight modification allowing us to run GUI applications in Docker
# while we are remoting/ssh'ed into the machine running the docker container.
# For example, docker container is running on Jetson Nano and I ssh into the Nano using a laptop.
# Through my laptop I want to run GUI applications from the docker container running on the Jetson Nano
# and see the application on my laptop. The change is at the end in the docker run command portion of this script.
# Resolved using following link: https://stackoverflow.com/questions/44429394/x11-forwarding-of-a-gui-app-running-in-docker
...
sudo docker run --runtime nvidia -it --rm --privileged --network host -e DISPLAY=$DISPLAY \
    ...
    $USER_VOLUME $CONTAINER_IMAGE bash
